...
(function setupControlOrdering() {
  ...
  function tryReorder() {
    ...
    // Geocoder may exist in either container depending on its configured position
    let geocoder = left.querySelector('.leaflet-control-geocoder');
    if (!geocoder && right) geocoder = right.querySelector('.leaflet-control-geocoder');

    const pdCtl   = left.querySelector('.pd-control[data-role="pd"]');
    const tzCtl   = left.querySelector('.pd-control[data-role="zones"]');
    const tripCtl = left.querySelector('.routing-control');
    const repCtl  = left.querySelector('.report-control');

    // Wait until all controls exist
    if (!geocoder || !pdCtl || !tzCtl || !tripCtl || !repCtl) return false;

    // If geocoder is in top-right, move it into the top-left stack first
    if (geocoder.parentNode !== left) {
      left.insertBefore(geocoder, left.firstChild);
    }
